const { Platform, ModuleType, StorageType, platformForId, platformForName } = require('./platform');
...
describe('Platform', () => {
	...
	describe('electron2', () => {
		it('has electron2 platform definitions by id', () => {
			const p = platformForId(37);
			const esomx = platformForName('esomx');
			expect(p).to.not.be.null;
			expect(esomx).to.not.be.null;

			expect(p.id).to.equal(37);
			expect(p.name).to.equal('electron2');
			expect(p.displayName).to.equal('Electron 2');

			expect(p).excluding(['_id', '_name', '_displayName']).to.deep.equal(esomx);
		});
		it('has electron2 platform definitions by name', () => {
			const p = platformForName('electron2');
			const esomx = platformForName('esomx');
			expect(p).to.not.be.null;
			expect(esomx).to.not.be.null;
