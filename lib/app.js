...
export class App {
	...
	async init(args) {
		...
		// Get module binaries
		const modules = await this._cache.getReleaseModules(version, { noCache: !args.cache });
	...
	async _flashDevices(devs, modules) {
		...
		for (let i = 0; i < devs.length; ++i) {
			const dev = devs[i];
			this._log.verbose(`${i + 1}. ${dev.id}`);
			const log = this._log.child(`[Device ${i + 1}]`);
			const f = new Flasher({ device: dev, dfu: this._dfu, usb: this._usb, log });
			flashers.push(f);
		}
	...
	async _listLocalDevices() {
		...
		this._log.verbose('Detected devices:');
		for (let i = 0; i < devs.length; ++i) {
