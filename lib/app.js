...
import { ModuleCache, ModuleType } from './module';
import { ParticleApi } from './api';
import { Flasher } from './flasher';
import { platformForId, platformForName } from './platform';
...
export class App {
	...
	async init(args) {
		...
		// Get module binaries
		let modules = await this._getModules(verOrPath, { noCache: !args.cache });
		...
		let devs = await this._openLocalDevices();
		devs = await this._getTargetDevices(devs, devArgs);
		// Close unused devices
		await Promise.all(devs.unused.map(dev => dev.close()));
		// Flash module binaries
		this._log.info('Flashing module binaries');
		devs = devs.target;
		await this._flashDevices(devs, modules);
		this._log.info('Done');
	}
	...
	async _flashDevices(devs, modules) {
		const flashers = [];
		...
		const promises = flashers.map(async (f) => {
			try {
				await f.flash(modules);
			} catch (err) {
	...
	async _openLocalDevices() {
		...
		await Promise.all(promises);
		if (error) {
			await Promise.all(devs.map(dev => dev.close()));
			throw error;
		}
		if (!devs.length) {
	...
	async _getModules(verOrPath, { noCache = false } = {}) {
		let mods = null;
		if (verOrPath.path) {
			const s = fs.statSync(verOrPath.path);
			if (s.isDirectory()) {
				mods = await this._cache.getModulesFromDirectory(verOrPath.path);
			} else {
				const mod = await this._cache.getModuleFromFile(verOrPath.path);
				mods = [mod];
			}
		} else {
			mods = await this._cache.getReleaseModules(verOrPath.version, { noCache });
		}
		if (!mods || !mods.length) {
			throw new Error('No module binaries found');
		}
		return mods;
	}
