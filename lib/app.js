...
export class App {
	...
	async init(args) {
		...
		let modules = [];
		if (verOrPath) {
			modules = await this._getModules(verOrPath, { noCache: !args.cache });
		...
		devs = devs.target;
		await this._flashDevices(devs, modules, { factoryReset: args['factory-reset'] });
		this._log.info('Done');
	}
	...
	async _flashDevices(devs, modules, { factoryReset = false } = {}) {
		const flashers = [];
		...
		const promises = flashers.map(async (f) => {
			try {
				await f.flash(modules, { factoryReset });
			} catch (err) {
