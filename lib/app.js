...
class App {
	...
	async init(args) {
		...
		const jobLimit = this._parseJobLimitArg(args);
		...
		let devs = await this._listLocalDevices({ jobLimit });
		devs = await this._getTargetDevices(devs, devArgs);
		// Flash module binaries
		this._log.info('Flashing target devices');
		await this._flashDevices(devs, modules, { jobLimit, factoryReset: args['factory-reset'] });
		this._log.info('Done');
	}
	...
	async _flashDevices(devs, modules, { factoryReset = false, jobLimit = Infinity } = {}) {
		const flashers = [];
	...
	async _listLocalDevices({ jobLimit = Infinity }) {
	...
	_parseJobLimitArg(args) {
		if (args.n === undefined) {
			return Infinity;
		}
		if (!Number.isInteger(args.n)) {
			throw new Error(`Invalid argument: ${args.n}`);
		}
		return args.n;
	}
