...
class App {
	...
	async init(args) {
		...
		let modules = [];
		if (verOrPath) {
			modules = await this._getModules(verOrPath, { noCache: !args.cache, noDraft: !args.draft });
	...
	async _getModules(verOrPath, { noCache = false, noDraft = true } = {}) {
		let mods = null;
		if (verOrPath.path) {
			mods = await this._cache.getModulesFromPath(verOrPath.path);
		} else {
			mods = await this._cache.getReleaseModules(verOrPath.version, { noCache, noDraft });
		}
		return mods;
	}
