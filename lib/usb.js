...
const FLASH_MODULE_TIMEOUT = 4 * 60 * 1000;

export async function openUsbDeviceById(id, { timeout = 3000 } = {}) {
	const t2 = Date.now() + timeout;
	for (;;) {
		try {
			const dev = usb.openDeviceById(id);
			await delay(250); // Just in case
			return dev;
		} catch (err) {
			// Ignore error
		}
...
export class UsbDevice extends Device {
	...
	async flashModule(module) {
		...
		await this._dev.updateFirmware(data, { timeout: FLASH_MODULE_TIMEOUT });
