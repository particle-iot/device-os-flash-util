...
const DEFAULT_TELNET_COMMAND_TIMEOUT = 2000;
...
class OpenOcdDevice extends Device {
	...
	async writeToFlash(file, storage, address) {
		...
		const resp = await this._command(`program ${file} ${addrStr}`, { timeout: 120000 });
...
export class OpenOcdFlashInterface extends FlashInterface {
	...
	async listDevices() {
		...
		for (let ad of adapters) {
			this._log.verbose(`${ad.index}. ${ad.info.displayName}; serial number: ${ad.serial}`);
		}
		return adapters.map(ad => new OpenOcdDevice({
			info: ad.info,
			serial: ad.serial,
			port: DEFAULT_TELNET_PORT + ad.index - 1,
			log: this._log.child(`[Adapter ${ad.index}]`)
		}));
	}
