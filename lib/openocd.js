...
class OpenOcdDevice extends Device {
	...
	async open(options) {
		...
		try {
			if (!this._target) {
				...
				this._target = await this._detectTarget();
				await delay(1000);
			}
	...
	async close() {
		try {
			if (this._ocdRunning) {
				this._ocdStopping = true;
				const ocdStopped = new Promise((resolve, reject) => {
					this.once('_ocdStopped', resolve);
				});
				if (this._telnetConnected) {
					...
					try {
						...
						await this._command('shutdown'); // This command always fails
					} catch (err) {
						// Ignore error
					}
				} else {
	...
	async writeToFlash(file, storage, address) {
		...
		if (this._target.unlockFlash) {
			const resp = await this._command(`flash write_image erase unlock ${file} ${addrStr}`, { timeout: FLASH_TIMEOUT });
			if (!resp.match(/wrote \d+ bytes from file/)) {
				throw new Error('Programming failed:\n' + resp);
			}
		} else {
			const resp = await this._command(`program ${file} ${addrStr}`, { timeout: FLASH_TIMEOUT });
	...
	async _detectTarget() {
		...
		const r = await execCommand('openocd', args, { timeout: 5000 });
		if (r.exitCode !== 0) {
			throw new Error(`OpenOCD exited with code ${r.exitCode}` + ('\n' + r.stderr).trimRight());
		}
		for (let gen of platformGen) {
			// TODO: This will likely not work for future platforms
			const p = platformCommonsForGen(gen);
			if (r.output.includes(p.openOcd.mcuManufacturer)) {
				return p.openOcd;
			}
		}
	...
	async _connectToOpenOcd() {
		...
		telnet.on('error', err => {
			if (this._telnetConnected && !this._ocdStopping) {
