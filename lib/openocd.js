...
const TELNET_CONNECT_TIMEOUT = 30000;
const DEFAULT_COMMAND_TIMEOUT = 30000;
const FLASH_COMMAND_TIMEOUT = 2 * 60 * 1000;

const MIN_OPENOCD_RESTART_INTERVAL = 3000;
const MIN_DEVICE_RESET_INTERVAL = 1000;
		...
		if (this._procRunning) {
			...
			if (this._telnetConnected && !this._cmdRunning) {
				try {
					...
					await this._telnet.send('shutdown', { timeout: DEFAULT_COMMAND_TIMEOUT });
...
class OpenOcdDevice extends Device {
	...
	async open(options) {
		...
		try {
			...
			await this._startOpenOcd(args, { resetTarget: this._target.resetTarget });
			if (!this.id) {
	...
	async close() {
		if (this._target.resetTarget && this._openocd.state === OpenOcdState.RUNNING && !this._openocd.isCommandRunning) {
	...
	async _detectPlatform() {
		try {
			...
			await this._startOpenOcd(args, { resetTarget: true });
	...
	async _startOpenOcd(args, { resetTarget = false }) {
		const now = Date.now();
		let startDelay = 0;
		let dt = now - this._lastStopped;
		if (dt < MIN_OPENOCD_RESTART_INTERVAL) {
			startDelay = MIN_OPENOCD_RESTART_INTERVAL - dt;
		}
		if (resetTarget) {
