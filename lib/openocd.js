...
const { platformCommons } = require('./platform');
const { isSpace, isPrintable, toUInt32Hex } = require('./util');
...
class OpenOcdDevice extends Device {
	...
	async writeToFlash(file, storage, address) {
		...
		if (this._platform.unlockFlash) {
			resp = await this._command(`flash write_image erase unlock ${file} ${addrStr}`, { timeout: FLASH_TIMEOUT });
		} else {
			resp = await this._command(`program ${file} ${addrStr}`, { timeout: FLASH_TIMEOUT });
		}
