...
import { openUsbDeviceById } from './usb';
import { ModuleType } from './module';
import { platformForId } from './platform';
import { execCommand, toUInt32Hex, toUInt16Hex } from './util';
...
export class DfuDevice extends Device {
	...
	async writeToFlash(file, storage, address) {
		...
		const path = usbDev.busNumber.toString() + '-' + usbDev.portNumbers.join('.');
		const args = [
			'-d', vidPid,
			'-p', path,
			'-a', alt.toString(),
