...
class ModuleCache {
	...
	async getReleaseModules(version, { noCache = false } = {}) {
		...
		if (!mods.length) {
			...
			mods = await this._downloadReleaseBinaries(version);
	...
	async _findMissingBinaries(version, modules) {
		...
		if (missingModules.size) {
			...
			for (let ver of versions) {
				...
				const mods = await this._downloadReleaseBinaries(ver);
				for (let m of mods) {
	...
	async _downloadDraftReleaseBinaries(rel, version) {
		// Since the release is untagged, it will need to be filtered by
		// the full list obtained from listReleaseAssets
		const assetsOfDev = await this._github.repos.listReleaseAssets ({
			owner: REPO_OWNER,
			repo: REPO_NAME,
			release_id: rel.id
		});
		const allAssets = assetsOfDev.data.map(a => ({
			url: a.url,
			file: a.name,
			size: a.size
		}));
		// From this point on, the code is compatible to function
		// with the existing code. Next steps are copy pasted
		...
		mkdirp.sync(destDir);
		// XXX: Currently fails here for draft releases with the following error -
		// HTTPError: Response code 404 (Not Found)
		// , but existing tagged releases pass this step
	...
	async _downloadReleaseBinaries(version) {
		let resp = null;
		try {
			...
			try {
				// Try a tag without 'v'
				resp = await this._github.repos.getReleaseByTag({ tag: version, repo: REPO_NAME, owner: REPO_OWNER });
			} catch (err) {
				if (err.status === 404) {
					console.log("TODO: since the release wasn't found. Try again, but instead hitting the (private) draft release API endpoint which required GITHUB_TOKEN");
					const listAllReleases = await this._github.repos.listReleases({
						owner: REPO_OWNER,
						repo: REPO_NAME
					});
					//console.log(listAllReleases.data);
					for (let rel of listAllReleases.data) {
          // FIXME: REMOVE THIS ASAP. HACK TO TEST 2.0.0 instead of 1.9.0-rc.1
						version = '2.0.0';
						if (rel.tag_name == String('v' + version)) {
							await this._downloadDraftReleaseBinaries(rel, version);
							break;
						}
					}
				}
	...
	async _downloadAssets(assets, dir) {
		...
		const promises = assets.map(a => limit(() => {
			...
			if (process.env.GITHUB_TOKEN) {
				console.log(`Using git hub token to download ${a.url} ${a.file} ${dir}`);
