...
class ModuleCache {
	...
	async _checkForDraftRelease(version) {
		let draftReleaseFound = false;
		console.log("Check for draft releases by hitting the (private) draft release API endpoint which requires GITHUB_TOKEN");
		...
		let rel = [];
		for (rel of listAllReleases.data) {
			//console.log('Release => ' + rel.tag_name);
			if (rel.tag_name == String('v' + version)) {
				console.log("Release found in the releases list. Check if it's draft");
				if (rel.draft == true) {
					draftReleaseFound = true;
					console.log("Release " + 'v' + version + " found. It is a draft release");
				}
				break;
			}
		}
		return draftReleaseFound ? rel : null;
	}
	...
	async _downloadReleaseBinaries(version) {
		...
		try {
			...
			try {
				...
				draftRel = await this._checkForDraftRelease(version);
				if (draftRel == null) {
					if (err.status === 404) {
		...
		let allAssets = [];
		if (draftRel) {
			// Get assets in a slightly different way for draft releases
			const assetsOfDraftRel = await this._github.repos.listReleaseAssets ({
				owner: REPO_OWNER,
				repo: REPO_NAME,
				release_id: draftRel.id
			});
			allAssets = assetsOfDraftRel.data.map(a => ({
				url: a.url,
				file: a.name,
				size: a.size
			}));
		} else {
			allAssets = resp.data.assets.map(a => ({
