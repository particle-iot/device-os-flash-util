...
class ModuleCache {
	...
	async getReleaseModules(version, { noCache = false, noDraft = true } = {}) {
		let mods = [];
		...
		if (!mods.length) {
			...
			const release = await this._downloadRelease(version, { noDraft });
			...
			if (!release.isDraft) {
				if (!noDraft) {
					this._log.warn(`Found the ${version} release but it is not a draft`);
				}
	...
	async _downloadRelease(version, { noDraft = true } = {}) {
		let release = null;
		let isDraft = false;
		try {
